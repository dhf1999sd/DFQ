...
module queue_manager#(
  ...
  reg  [9:0]                 r_ptr_ram_addr [NUM_PRIORITY-1:0];
  wire [9:0]                 w_ptr_ram_rd_addr [NUM_PRIORITY-1:0];
  ...
  wire [5:0]                 w_prio_queue_count [NUM_PRIORITY-1:0];
  ...
  assign w_prio_queue_rd_en   = w_arb_grant_out;
  ...
  generate  // Cyclic treatment of different PCPs
    ...
    begin : FMT_Different_PCP
      FMT #(
            ...
            .i_search_req           (w_fmt_in_wr[p] & w_fmt_in_data[p][14]),
    ...
    begin
      ...
      case (r_flow_wr_state)
        ...
        begin
          if (r_ptr_wr_req)
          begin
            r_flow_wr_state <= w_read_busy[r_wr_prio_idx] ? 0 : 3;
            r_ptr_wr_nack    <= w_read_busy[r_wr_prio_idx];
            if (r_ptr_din[14])
            begin
              if (w_fmt_search_hit[r_wr_prio_idx] && !w_read_busy[r_wr_prio_idx])
              begin
                r_flow_tail[r_wr_prio_idx]       <= w_fmt_match_tail[r_wr_prio_idx];
                r_write_busy[r_wr_prio_idx] <= 1'b1;
              end
              else if (w_fmt_search_miss[r_wr_prio_idx] && !w_read_busy[r_wr_prio_idx])
              begin
        ...
        begin
          ...
          begin
            ...
            r_ptr_ram_addr[r_wr_prio_idx][9:0] <= r_flow_tail[r_wr_prio_idx][9:0];
            r_ptr_ram_din[r_wr_prio_idx][19:0] <= r_ptr_din[19:0];
            r_flow_tail[r_wr_prio_idx]              <= r_ptr_din;
          end
          else
          begin
            if (w_fmt_search_miss[r_wr_prio_idx] == 1'b1)
            begin
              r_ptr_ram_wr_en[r_wr_prio_idx]        <= 1'b1;
              r_ptr_ram_addr[r_wr_prio_idx][9:0] <= r_ptr_din[9:0];
              r_ptr_ram_din[r_wr_prio_idx][19:0] <= r_ptr_din[19:0];
              r_flow_tail[r_wr_prio_idx]              <= r_ptr_din;
              r_flow_head[r_wr_prio_idx]              <= r_ptr_din;
            end
            else
            begin
              r_ptr_ram_wr_en[r_wr_prio_idx]        <= 1'b1;
          ...
          begin
            ...
            if (w_fmt_search_miss[r_wr_prio_idx])
            begin
              r_fmt_alloc_tail_data[r_wr_prio_idx]     <= r_ptr_din;
              r_fmt_alloc_tail_req[r_wr_prio_idx] <= 1'b1;
            end
            else
            begin
              r_fmt_tail_update_req[r_wr_prio_idx] <= 1'b1;
              r_fmt_refresh_tail[r_wr_prio_idx]  <= r_ptr_din;
            end
          end
        end
        2:
        begin
          r_flow_wr_state               <= 0;
          r_ptr_ram_addr[r_wr_prio_idx]    <= r_flow_tail[r_wr_prio_idx][9:0];
  ...
  generate
    ...
    begin : FMT_Different_RAM
      sram_FMT u_flow_ram (
                 ...
                 .addra  (r_ptr_ram_addr[q][8:0]),  // input wire [7 : 0] addra
                 .dina   (r_ptr_ram_din[q]       ),  // input wire [19 : 0] dina
                 .clkb   (clk                  ),  // input wire clkb
                 .addrb  (w_ptr_ram_rd_addr[q]   ),  // input wire [7 : 0] addrb
